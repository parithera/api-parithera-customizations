...
import { Group, Request, ResponseData } from "../types";
...
export class ScanpyToolService {
    ...
    async parseScanpyAnswer(answer: string, response_data: ResponseData, data: Request): Promise<ResponseData> {
    ...
    async executeScript(organizationId: string, projectId: string, user: AuthenticatedUser, response_data: ResponseData): Promise<ResponseData> {
        ...
        if (!fs.existsSync(filePath)) {
            throw new Error(`File ${filePath} did not exist after 2 minutes.`);
        }
